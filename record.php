...
require('connect.php');
$dataPerPage = 20;
$totalData = count(query("SELECT * FROM esp32_record WHERE sensor = 1"));
$totalPage = ceil($totalData / $dataPerPage);
$activePage = (isset($_GET["page"])) ? $_GET["page"] : 1;
$begin = ($dataPerPage * $activePage) - $dataPerPage;

$data1 = mysqli_query($connect, "SELECT kelembapan, suhu, gas, waktu, time(waktu) as jam FROM esp32_record WHERE sensor = 1 ORDER BY jam DESC LIMIT $begin, $dataPerPage ") or die(mysqli_error($connect));
$data2 = mysqli_query($connect, "SELECT kelembapan, suhu, gas, waktu, time(waktu) as jam FROM esp32_record WHERE sensor = 2 ORDER BY jam DESC LIMIT $begin, $dataPerPage ") or die(mysqli_error($connect));
$data3 = mysqli_query($connect, "SELECT kelembapan, suhu, gas, waktu, time(waktu) as jam FROM esp32_record WHERE sensor = 3 ORDER BY jam DESC LIMIT $begin, $dataPerPage ") or die(mysqli_error($connect));
